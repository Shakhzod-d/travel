        ...
        <div className='w-full flex flex-wrap gap-x-11 gap-y-5 justify-center lg:justify-start mb-5'>
            {
                data.results.map((item: CountriesType) => (
                    <button 
                        ...
                        key={item.id} 
                        id={item.title} 
                        className={`btn px-[24px] py-[12px] ${activeCountry.length !== 0 ? item.title == activeCountry ? 'bg-[#F56960] text-white' : 'text-black bg-[#F5F5F5]' : data.results.indexOf(item) == 0 && 'bg-[#F56960] text-white'}`}>
                            {item.title}
                    </button>
                ))
            }
