...
const ContactFormSection = () => {
  ...
  const { bookingModal, serviceId, tourModal, id1, start, end, persons } =
    state;
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: Data) => {
    setIspending(true);
    await axios
      .post(`${Base_URL}/api/booking/v1/service/${serviceId}/`, data)
      .then((_) => {
        Swal.fire({
          position: "center",
          icon: "success",
  ...
  const onSubmit2 = async (data: Data) => {
    ...
    setIspending(true);
    await axios
      .post(`${Base_URL}/api/booking/v1/tour/${id1}/`, bookingTourData)
      .then((_) => {
        Swal.fire({
          position: "center",
          icon: "success",
  ...
  return (
    <div
      className={`w-full animate__animated animate__zoomIn h-screen fixed justify-center items-center z-50 pointer-events-none break-words ${
        bookingModal ? "flex" : "animate__animated animate__zoomOut"
      }`}
    >
      <div className="flex justify-center items-center w-full h-screen pointer-events-none">
        <div className="pointer-events-auto rounded-2xl shadow-lg bg-white h-max p-5 max-w-[400px] w-full">
          ...
          <form
            onSubmit={
              tourModal
                ? handleSubmit((data) => onSubmit2(data as BookingTourData))
                : handleSubmit(onSubmit)
            }
            className="p-2 max-w-[540px] xl:max-w-[450px] lg:max-w-[540px] w-full bg-white"
          >
            <div>
            ...
            <div>
              <h3 className="text-[18px] font-semibold mb-2">
                {tourModal ? t("comment") : t("whathelp")}
              </h3>
              <textarea
                id="content"
            ...
            <button
              type="submit"
              className="bg-[#635AFF] cursor-pointer rounded-md p-3 w-full h4 mt-2"
            >
              {isPending ? (
