...
const ServiceCart = ({ image, title, content, id }: ServiceCartType) => {
    ...
    return (
        <div className="bg-white rounded-[16px] w-full min-h-[368px] flex flex-col items-center justify-between px-[45px] py-11 shadow-lg">
            ...
            <h4 className="h4 text-[#818181] font-[500] w-full text-center md:my-2">{sanitize(content)}</h4>
            <button
                id={id.toString()}
                onClick={handleBooking}
                className="btn mt-2 text-[#F56960] border-[1px] border-[#F56960] hover:bg-[#F56960] hover:text-white text-[16px] font-[500] px-[80px] xl:px-[50px] md:px-[20px] py-2"
            >
                {t("booknow")}
            </button>
        </div> 
